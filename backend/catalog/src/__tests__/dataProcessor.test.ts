import { 
  processMovieData, 
  batchProcess, 
  sanitizeInput, 
  parseRating,
  ProcessingResult 
} from '../utils/dataProcessor';

describe('Data Processor', () => {
  describe('processMovieData', () => {
    it('should process valid items successfully', () => {
      const items = [1, 2, 3, 4, 5];
      const processor = (item: number) => item * 2;
      
      const result = processMovieData(items, processor);
      
      expect(result.data).toEqual([2, 4, 6, 8, 10]);
      expect(result.processed).toBe(5);
      expect(result.errors).toEqual([]);
    });

    it('should handle null returns from processor', () => {
      const items = [1, 2, 3, 4, 5];
      const processor = (item: number) => item % 2 === 0 ? item * 2 : null;
      
      const result = processMovieData(items, processor);
      
      expect(result.data).toEqual([4, 8]);
      expect(result.processed).toBe(2);
      expect(result.errors).toEqual([]);
    });

    it('should capture errors from processor', () => {
      const items = [1, 2, 3];
      const processor = (item: number) => {
        if (item === 2) throw new Error('Processing error');
        return item * 2;
      };
      
      const result = processMovieData(items, processor);
      
      expect(result.data).toEqual([2, 6]);
      expect(result.processed).toBe(2);
      expect(result.errors).toEqual(['Processing error']);
    });

    it('should handle non-Error exceptions', () => {
      const items = [1, 2];
      const processor = (item: number) => {
        if (item === 2) throw 'String error';
        return item * 2;
      };
      
      const result = processMovieData(items, processor);
      
      expect(result.data).toEqual([2]);
      expect(result.processed).toBe(1);
      expect(result.errors).toEqual(['Unknown error']);
    });

    it('should handle empty input array', () => {
      const items: number[] = [];
      const processor = (item: number) => item * 2;
      
      const result = processMovieData(items, processor);
      
      expect(result.data).toEqual([]);
      expect(result.processed).toBe(0);
      expect(result.errors).toEqual([]);
    });
  });

  describe('batchProcess', () => {
    it('should process items in batches', async () => {
      const items = [1, 2, 3, 4, 5, 6, 7, 8];
      const batchSize = 3;
      const processor = async (batch: number[]) => batch.map(x => x * 2);
      
      const result = await batchProcess(items, batchSize, processor);
      
      expect(result).toEqual([2, 4, 6, 8, 10, 12, 14, 16]);
    });

    it('should handle single batch', async () => {
      const items = [1, 2, 3];
      const batchSize = 5;
      const processor = async (batch: number[]) => batch.map(x => x * 2);
      
      const result = await batchProcess(items, batchSize, processor);
      
      expect(result).toEqual([2, 4, 6]);
    });

    it('should handle empty input', async () => {
      const items: number[] = [];
      const batchSize = 3;
      const processor = async (batch: number[]) => batch.map(x => x * 2);
      
      const result = await batchProcess(items, batchSize, processor);
      
      expect(result).toEqual([]);
    });

    it('should handle batch size of 1', async () => {
      const items = [1, 2, 3];
      const batchSize = 1;
      const processor = async (batch: number[]) => batch.map(x => x * 2);
      
      const result = await batchProcess(items, batchSize, processor);
      
      expect(result).toEqual([2, 4, 6]);
    });
  });

  describe('sanitizeInput', () => {
    it('should trim whitespace', () => {
      expect(sanitizeInput('  hello world  ')).toBe('hello world');
    });

        it('should remove HTML tags', () => {
      expect(sanitizeInput('<div>hello</div>')).toBe('hello');
      expect(sanitizeInput('<script>alert("xss")</script>content')).toBe('alert("xss")content');
    });

    it('should remove script keywords', () => {
      expect(sanitizeInput('hello script world')).toBe('hello  world');
      expect(sanitizeInput('SCRIPT tag')).toBe(' tag');
    });

    it('should limit length to 1000 characters', () => {
      const longString = 'a'.repeat(1500);
      const result = sanitizeInput(longString);
      expect(result).toHaveLength(1000);
    });

    it('should handle empty string', () => {
      expect(sanitizeInput('')).toBe('');
    });

    it('should handle complex HTML with scripts', () => {
      const input = '  <div><script>evil()</script>Good content</div>  ';
      expect(sanitizeInput(input)).toBe('evil()Good content');
    });
  });

  describe('parseRating', () => {
    it('should return number as-is when within range', () => {
      expect(parseRating(5)).toBe(5);
      expect(parseRating(0)).toBe(0);
      expect(parseRating(10)).toBe(10);
    });

    it('should clamp numbers outside range', () => {
      expect(parseRating(15)).toBe(10);
      expect(parseRating(-5)).toBe(0);
    });

    it('should parse valid string numbers', () => {
      expect(parseRating('5.5')).toBe(5.5);
      expect(parseRating('8')).toBe(8);
      expect(parseRating('0')).toBe(0);
    });

    it('should clamp parsed string numbers', () => {
      expect(parseRating('15')).toBe(10);
      expect(parseRating('-5')).toBe(0);
    });

    it('should return 0 for invalid strings', () => {
      expect(parseRating('not-a-number')).toBe(0);
      expect(parseRating('')).toBe(0);
      expect(parseRating('abc')).toBe(0);
    });

    it('should handle edge cases', () => {
      expect(parseRating('10.1')).toBe(10);
      expect(parseRating('-0.1')).toBe(0);
    });
  });
});
