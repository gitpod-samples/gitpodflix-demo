...
export const createApp = (pool?: Pool) => {
  ...
  // Use provided pool or create default one
  ...
  app.get('/api/movies', async (req, res) => {
    try {
      const result = await dbPool.query('SELECT * FROM movies ORDER BY rating DESC');
  ...
  app.post('/api/movies/clear', async (req, res) => {
    try {
      await dbPool.query('TRUNCATE TABLE movies');
